...
public interface OverclockedCableService extends IGridNodeService {
    /**
     * @return The base channel capacity this tier provides, before {@link AeocConfig#CHANNEL_CAPACITY_MULTIPLIER} and
    ...
    static int resolveMaxChannels(OverclockedCableService service, ChannelMode channelMode) {
        if (AeocConfig.UNLIMITED_CHANNELS.get() || channelMode == ChannelMode.INFINITE) {
            return Integer.MAX_VALUE;
        }
        var scaled = service.getBaseChannelCapacity() * AeocConfig.CHANNEL_CAPACITY_MULTIPLIER.get();

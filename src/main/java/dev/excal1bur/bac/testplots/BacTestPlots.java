package dev.excal1bur.bac.testplots;
 ...
 */
@TestPlotClass
public final class BacTestPlots {
    private BacTestPlots() {
    }
    ...
    public static void overclockedSmartCable16Channels(PlotBuilder plot) {
        var cableItem = BACParts.get(BacTiers.OVERCLOCKED, CableShape.SMART, AEColor.TRANSPARENT).get();
        var expectedCapacity = BacTiers.OVERCLOCKED.capacityFor(CableShape.SMART);
    ...
    public static void hyperconductiveDenseCable128Capacity(PlotBuilder plot) {
        var cableItem = BACParts.get(BacTiers.HYPERCONDUCTIVE, CableShape.SMART_DENSE, AEColor.TRANSPARENT).get();
        var expectedCapacity = BacTiers.HYPERCONDUCTIVE.capacityFor(CableShape.SMART_DENSE);
